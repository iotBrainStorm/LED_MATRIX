...
String processTemplate(const String& tmpl) {
  ...
  out.replace("{AMPM}", (t.tm_hour >= 12) ? "PM" : "AM");
  ...
  const char* monthsShort[] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
  const char* monthsLong[]  = {"January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"};
  out.replace("{MMM}", monthsShort[t.tm_mon % 12]);
  out.replace("{MMMM}", monthsLong[t.tm_mon % 12]);

  int year = t.tm_year + 1900;
  snprintf(buf, sizeof(buf), "%02d", year % 100);
  out.replace("{YY}", buf);
  snprintf(buf, sizeof(buf), "%d", year);
  out.replace("{YYYY}", buf);

  const char* daysShort[] = {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"};
  const char* daysLong[]  = {"Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"};
  out.replace("{WWW}", daysShort[t.tm_wday % 7]);
  out.replace("{WWWW}", daysLong[t.tm_wday % 7]);

  snprintf(buf, sizeof(buf), "%.1f", temp);
  out.replace("{TEMP}", buf);
...
void loadConfiguration() {
  ...
  if (scenesArr.isNull() || scenesArr.size() == 0) return;
